    ...
    public static int[][] getNearLessNoRepeat(int[] arr) {
        ...
        for (int i = 0; i < arr.length; i++) {
            while (!stack.isEmpty() && arr[stack.peek()] > arr[i]) {
                ...
                res[popIdx] = new int[]{leftLessIdx, i};
            }
        ...
        while (!stack.isEmpty()) {
            Integer popIdx = stack.pop();
            int leftLessIdx = stack.isEmpty() ? -1 : stack.peek();
            res[popIdx] = new int[]{leftLessIdx, -1};
        }
        return res;
    }
        ...
        for (int i = 0; i < arr.length; i++) {
            while (!stack.isEmpty() && arr[stack.peek().get(0)] > arr[i]) {
                ...
                for (Integer popIndex : popIndexes) {
                    res[popIndex] = new int[]{leftLessIdx, i};
                }
            }
            if (!stack.isEmpty() && arr[stack.peek().get(0)] == arr[i]) {
                stack.peek().add(i);
            } else {
                List<Integer> cur = Lists.newArrayList(i);
                stack.push(cur);
            }
        }
        while (!stack.isEmpty()) {
